...
class dem(object):
    ...
    def fill_depression(self,level_max=100,method=0,msg=False):
        '''
    ...
    def proc_demfile(self,names=None,ids=None,sname=None,sdir='.',findex=None,depth_limit=[-1e5,1e4],subdomain_size=1e6,offset=1):
        '''
...
#------------------------------------------------------------------------------
    ...
    #ignore id number
    names=['GEBCO.asc',]; sname='A';
    npt_sample=2; npt_smooth_cmb=3

    S=dem()
    if not os.path.exists('{}.npz'.format(sname)):
       S.proc_demfile(names,sname=sname,depth_limit=[0,5000],subdomain_size=1e7)
    S.read_data('{}.npz'.format(sname))
    S.compute_watershed()
    S.compute_river(area_limit=1e8)
    S.write_shapefile('{}'.format(sname),npt_smooth=20)
    S.combine_shp(sname,npt_sample=npt_sample,npt_smooth=npt_smooth_cmb)
    sys.exit()
